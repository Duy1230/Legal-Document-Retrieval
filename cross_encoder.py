...
class CrossEncoderWrapper:
    def __init__(
        ...
        self.model = CrossEncoder(
            ...
            device=None  # Will automatically use GPU if available
        )
    ...
    def train(self, train_dataset: Dataset, val_dataset: Dataset = None,
        ...
        if val_dataset is not None:
            ...
            evaluator = CEBinaryAccuracyEvaluator.from_input_examples(val_samples, name="evaluation")
        else:
            evaluator = None
        # Create callback
        ...
        self.model.fit(
            ...
            evaluator=evaluator,
            epochs=num_epochs,
            evaluation_steps=2000,  # Evaluate every 100 steps
    ...
    def predict(self, questions: List[str], documents: List[str], show_progress_bar = True) -> np.ndarray:
        ...
        return self.model.predict(pairs, show_progress_bar=show_progress_bar)
...
class CrossEncoderDataset(Dataset):
    def __init__(self, questions: List[str], documents: List[str], labels: List[int]):
        ...
        self.labels = labels
        assert len(questions) == len(documents) == len(labels), "All inputs must have the same length"
